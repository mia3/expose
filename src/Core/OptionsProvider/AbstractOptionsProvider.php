    ...
    public function __construct($propertySchema, $settings = array())
    {
        ...
        if (($unsupportedSettings = array_diff_key($settings, $this->supportedSettings)) !== array()) {
            ...
            foreach (array_keys($unsupportedSettings) as $unsupportedSetting) {
                foreach ($supportedSettings as $supportedSetting) {
                    ...
                    if ($similarity > 50) {
                        throw new \Exception(sprintf('The settings "%s" you\'re trying to set don\'t exist, did you mean: "%s" ?.',
                            $unsupportedSetting, $supportedSetting), 1359625876);
                    }
                }
            }
            throw new \Exception(sprintf('The settings "%s" you\'re trying to set don\'t exist in class "%s".',
                implode(',', array_keys($unsupportedSettings)), get_class($this)), 1359625876);
        }
        ...
        array_walk(
            ...
            function ($supportedSettingData, $supportedSettingName, $settings) {
                if (isset($supportedSettingData['required']) && $supportedSettingData['required'] == true && !array_key_exists($supportedSettingName,
                        $settings)
                ) {
                    throw new \Exception('Missing required OptionsProvider setting: ' . $supportedSettingName . chr(10) . $supportedSettingData['description'],
                        1359625876);
                }
            },
            $settings
