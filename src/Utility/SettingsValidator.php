                ...
                foreach ($supportedSettings as $supportedSetting) {
                    ...
                    if ($similarity > 50) {
                        throw new \Exception(sprintf('The settings "%s" you\'re trying to set don\'t exist, did you mean: "%s" ?.',
                            $unsupportedSetting, $supportedSetting), 1407785248);
                    }
                }
            }
            throw new \Exception(sprintf('The settings "%s" you\'re trying to set don\'t exist.',
                implode(',', array_keys($unsupportedSettings))), 1407785250);
        }
        ...
        array_walk(
            ...
            function ($supportedSettingData, $supportedSettingName, $settings) {
                if (isset($supportedSettingData['required']) && $supportedSettingData['required'] == true && !array_key_exists($supportedSettingName,
                    ...
                    throw new \Exception('Missing required setting: ' . $supportedSettingName . chr(10) . $supportedSettingData['description'],
                        1407785252);
                }
            },
            $settings
